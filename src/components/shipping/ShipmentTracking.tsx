...
export const ShipmentTracking: React.FC<Props> = ({ shipments, isLoading }) => {
  ...
  return (
    <div className="rounded border border-brand-border bg-brand-card p-6 shadow-lg h-full text-left">
      ...
      {shipments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {shipments.map((shipment, idx) => {
            const lat = shipment.vessel?.latitude ?? 20.27;
            const lng = shipment.vessel?.longitude ?? 86.67;
            const speed = shipment.vessel?.current_speed ?? 14.2;
            const vesselName = shipment.vessel?.vessel_name || 'Unassigned Vessel';
